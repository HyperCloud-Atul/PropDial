...
const FullInspection = ({ inspectionId, inspectionDocument }) => {
...
// UI helper functions for rooms - Updated version
const getRoomClass = (roomId) => {
  ...
  if (room.isAllowForInspection === "no") {
    ...
    if (totalFixtures === 0) {
      ...
      // Agar koi bhi fixture partially bhi filled hai to room "In Progress" ho jayega
      if (partiallyInspectedFixtures > 0 && inspectedFixtures < totalFixtures) {
        className += " half";
      } else if (inspectedFixtures === totalFixtures) {
        className += " full";
      } else {
        className += " add";
      }
...
// UI helper functions for fixtures - Updated version
