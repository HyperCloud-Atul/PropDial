...
const MyProperties = () => {
  ...
  const { documents: mState, errors: mStateError } = useCollection("m_states", [
    "status",
    "==",
    "active",
  ]);
...
if (user?.role === "owner") {
...
} else if (user?.role === "manager" || user?.role === "executive") {
  ...
  useEffect(() => {
    if (!selectedCity || !user?.phoneNumber) return;

    setLoading(true);

    const fetchProperties = async () => {
      try {
        // Step 1: Get property IDs assigned to the user
        const assignedSnapshot = await projectFirestore
          .collection("propertyusers")
          .where("userId", "==", user.phoneNumber)
          .where("userType", "==", userType)
          .get();

        const assignedPropertyIds = assignedSnapshot.docs.map(
        ...
        // Step 2: Filter properties by city AND assignedPropertyIds
        const chunkSize = 10;
        const chunks = [];

        for (let i = 0; i < assignedPropertyIds.length; i += chunkSize) {
          chunks.push(assignedPropertyIds.slice(i, i + chunkSize));
        }
        ...
        <div className="top_header_pg pg_bg pg_adminproperty">
          <div className="page_spacing pg_min_height">
            ...
            <div className="pg_header d-flex justify-content-between">
              <div className="left">
                <h2 className="m22 text-capitalize">
                  {user?.role} Filtered properties:{" "}
                  {filteredProperties && (
            ...
            <div className="filters">
              <div className="left">
                <Select
                  ...
                  value={
                    stateOptions.find((opt) => opt.value === selectedStateId) ||
                    null
                  }
