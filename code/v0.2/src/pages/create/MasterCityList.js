...
export default function MasterCityList() {
    ...
    const { updateDocument, response: responseUpdateDocument } = useFirestore('m_cities')
    ...
    const filteredData = masterCity ? masterCity.filter(document => {
        let isfiltered = false
        switch (filter) {
            case 'INDIA':
                return document.country === 'INDIA' ? isfiltered = true : isfiltered

            case 'USA':
                return document.country === 'USA' ? isfiltered = true : isfiltered
    ...
    return (
        <div>
            ...
            <hr></hr>
            {filteredData && <Filters changeFilter={changeFilter} filterList={dataFilter} filterLength={filteredData.length} />}
            <div className="row no-gutters">
                            ...
                            <div className="profile-card-div" style={{ position: 'relative' }} >
                                <div className="address-div" style={{ paddingBottom: '5px' }}>
                                    ...
                                    <div className="address-text" >
                                        <div onClick={() => handleEditCard(data.id, data.country, data.state, data.city)}
                                            style={{
                                                width: '80%',
                                                height: '170%',
                                                textAlign: 'left',
                                            ...
                                            <h5 style={{ margin: '0', transform: 'translateY(5px)' }}>{data.city}</h5>
                                            <small style={{ margin: '0', transform: 'translateY(5px)' }}>{data.state}, {data.country}</small>
                                        </div>
