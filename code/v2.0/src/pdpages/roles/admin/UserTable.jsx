...
const UserTable = ({ users, filter, isExEmployee }) => {
  ...
  const columns = useMemo(() => {
    const baseColumns = [
        ...
        Cell: ({ row }) => {
          ...
          return (
            <div className="pointer mobile_min_width">
              ...
              <Link 
                to={`/profiledetails/${phone}${isExEmployee ? '?type=exEmployee' : ''}`} 
                className="text-capitalize text_light_black"
              >
      ...
      {
        Header: "Country",
        accessor: "residentialCountry",
        disableSortBy: false,
        Cell: ({ row }) => {
          const rawCountry = row.original.residentialCountry || row.original.country;
          const formatted = formatCountry(rawCountry);
          return <div className="mobile_min_width">{formatted}</div>;
        },
      },
      {
        Header: "City",
    ...
    // ✅ Inject Cities Owned column only when filter is "Owner" and not ex-employee
    if (filter === "Owner" && !isExEmployee) {
      baseColumns.splice(2, 0, {
        Header: "Property City",
        accessor: (row) => {
          if (!row.propertiesOwnedInCities || row.propertiesOwnedInCities.length === 0) return "";
          return row.propertiesOwnedInCities.map(city => city.label).join(", ");
        },
        id: "citiesOwned",
        disableSortBy: true,
        Cell: ({ value }) => (
    ...
    return baseColumns;
  }, [filter, isExEmployee]); // 👈 include `isExEmployee` in dependency
