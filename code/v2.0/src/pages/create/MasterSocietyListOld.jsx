...
const [city, setCity] = useState({ label: "Select City", value: "" });
const [locality, setLocality] = useState({ label: "Select Locality", value: "" });
  ...
  if (masterCountry) {
    ...
    // Initial values set karein with select options
    setCountry({ label: "INDIA", value: "_india" });
    setState({ label: "Select State", value: "" }); // Select State as default
    setCity({ label: "Select City", value: "" });
    setLocality({ label: "Select Locality", value: "" });
    
    // Initial options setup
    stateOptions.current = [{ label: "Select State", value: "" }];
    cityOptions.current = [{ label: "Select City", value: "" }];
    localityOptions.current = [{ label: "Select Locality", value: "" }];
  ...
  const handleCountryChange = async (option) => {
  setCountry(option);
  setState({ label: "Select State", value: "" }); // Reset to Select State
  setCity({ label: "Select City", value: "" });
  setLocality({ label: "Select Locality", value: "" });
      ...
      async (snapshot) => {
        if (snapshot.docs && snapshot.docs.length > 0) {
          stateOptions.current = [
            { label: "Select State", value: "" }, // Default option add karein
            ...snapshot.docs.map((stateData) => ({
              label: stateData.data().state,
              value: stateData.id,
            }))
          ];
        ...
        // City aur locality options reset karein
        cityOptions.current = [{ label: "Select City", value: "" }];
        localityOptions.current = [{ label: "Select Locality", value: "" }];
      },
      (err) => {
  ...
  const handleStateChange = async (option) => {
  setState(option);
  setCity({ label: "Select City", value: "" }); // Reset city
  setLocality({ label: "Select Locality", value: "" }); // Reset locality
  ...
  if (option && option.value) {
    ...
    ref.onSnapshot(
      async (snapshot) => {
        if (snapshot.docs) {
          cityOptions.current = [
            { label: "Select City", value: "" }, // Default option add karein
...
const handleCityChange = async (option) => {
  ...
  if (option && option.value) {
    ...
    ref.onSnapshot(
      async (snapshot) => {
        if (snapshot.docs) {
          localityOptions.current = [
            { label: "Select Locality", value: "" }, // Default option add karein
...
<div>
  ...
  placeholder={state && state.value ? "Select City" : "First select state"}
                      styles={{
                        control: (baseStyles, state) => ({
                          ...baseStyles,
                          outline: "none",
  ...
  placeholder={city && city.value ? "Select Locality" : "First select city"}
                        value={locality}
                        styles={{
                          control: (baseStyles, state) => ({
                            ...baseStyles,
