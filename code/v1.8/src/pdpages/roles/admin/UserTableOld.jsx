...
const columns = useMemo(() => {
  const baseColumns = [
    ...
    //   accessor: "createdAt",
    //   disableSortBy: true,
    //   Cell: ({ value }) => <div className="mobile_min_width">{format(value.toDate(), "dd-MMM-yy hh:mm a")}</div>,
    // },
    // {
    //   Header: "Last Login",
    ...
    //   disableSortBy: true,
    //   Cell: ({ value }) => (
    //     <span className={`text-capitalize ${value === "active" ? "text_green2" : "text_red"}`}>{value}</span>
    //   ),
    // },
    {
  Header: "City / Country",
...
if (filter === "Owner") {
  baseColumns.splice(2, 0, 
    ...
    filterValue: row => {
      ...
      return row.map(city => city.label.toLowerCase()).join(", ");
    },
  }
);
}
