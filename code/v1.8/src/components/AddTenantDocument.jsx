...
const AddTenantDocument = () => {
    ...
    try {
      ...
      if (docType === "rent") {
        ...
        newDoc = {
          ...newDoc,
          docType,
          fileType: "rent-agreement", // custom type
          rentPdf: {
  ...
  const handleDelete = async (index) => {
    ...
    try {
      if (docToDelete.docType === "rent") {
        if (docToDelete.rentPdf?.fileName) {
          await projectStorage
            .ref(`tenantDocs/${tenantId}/${docToDelete.rentPdf.fileName}`)
            .delete();
        }
        if (docToDelete.rentWord?.fileName) {
          await projectStorage
            .ref(`tenantDocs/${tenantId}/${docToDelete.rentWord.fileName}`)
            .delete();
        }
      } else {
