...
const PropertyTable = ({ properties }) => {
  ...
  // usee effect to avoid re-rendering the table on every change 
//   useEffect(() => {
//     if (!properties || properties.length === 0) return;
//  let isMounted = true;
//     const initialData = properties.map((property) => ({
...
//     try {
//       console.log("📥 Reading user doc:", userId); // 🔍 Log each user read
//       const userDoc = await projectFirestore
//         .collection("users-propdial")
//         .doc(userId)
//         .get();
...
//     for (const property of updatedData) {
//       console.log("📘 Reading roles for property:", property.id); // 🔍 Log propertyusers read
//       const rolesSnapshot = await projectFirestore
//         .collection("propertyusers")
//         .where("propertyId", "==", property.id)
//         .get();
...
//         if (type === "propertyowner" && (tag === "owner" || tag === "co-owner")) {
//           property.owner = userData;
//         }
//         if (type === "propertymanager" && tag === "executive") {
//           property.executive = userData;
...
useEffect(() => {
  ...
  let totalReads = 0; // ✅ Track total reads
  ...
  const fetchUserDetails = async (userId) => {
    ...
    try {
      console.log("📥 Reading user doc:", userId);
      const userDoc = await projectFirestore
        .collection("users-propdial")
        .doc(userId)
        .get();

      totalReads++; // ✅ Count 1 user doc read
  ...
  const fetchRoles = async () => {
    ...
    for (const property of updatedData) {
      console.log("📘 Reading roles for property:", property.id);
      ...
      totalReads += rolesSnapshot.size || 1; // ✅ Count all propertyusers reads (1 read per document fetched)
    ...
    console.log("✅ Total Firestore Reads:", totalReads); // ✅ Final Read Count Here
  };
