...
const ReferralLogin = () => {
  ...
  const { referralCode, referredBy } = useParams();
  // Example console log
  ...
  const { updateDocument: updateReferralDoc, errors } =
  ...
  const handleNewUserData = async () => {
    ...
    try {
      ...
      if (user.id) {
        await updateReferralDoc(referralCode, {
          referTo: user.uid,
          isAccept: true,
        });
      } else {
  ...
  //     // Generate a document reference to get the ID
  //     const docRef = projectFirestore.collection("users-propdial").doc();
        ...
        <div>
          {otpSliderState && (
            <div className="left_inner col_left_inner">
              ...
              >
                <label htmlFor="" className="w-100 relative">
                  Enter 6 digit OTP
                  <span
